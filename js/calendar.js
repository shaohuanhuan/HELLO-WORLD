...
Calender.prototype = {
    ...
    _template :[
        ...
        '<div class="ui-today-wrap"><h4 class="ui-today">18</h4><p>周日</p></div>',
        '</dt>',
        '<dt>周日</dt>',
        '<dt>周一</dt>',
        '<dt>周二</dt>',
    ...
    createContainer:function(){
       ...
       // container.style.top = inputPos.bottom - 1 + 'px';
        ...
        $('#'+this.id).parent('#j-date').append(container);
    },
    // 渲染日期
    drawDate:function (odate) { // 参数 odate 为日期对象格式
        ...
        var nowDate = new Date(),nowyear = nowDate.getFullYear(),nowmonth = nowDate.getMonth(),
            ...
            weekshow=new Array("周日", "周一", "周二", "周三", "周四", "周五", "周六")[nowDate.getDay()];
        ...
        //sh-显示
        $('.ui-today-wrap h4').text(nowdate);
        $('.ui-today-wrap p').text(weekshow);
        //IE6 select遮罩
    ...
    btnEvent:function(){
        //var prevyear = _CalF.$('.prevyear',this.dateWarp)[0],
         var   prevmonth = _CalF.$('.prevmonth',this.dateWarp)[0],
    ...
    linkOn:function(){
        ...
        for(i = 0;i<l;i++){
            ...
            links[i].onclick = function(){
                ...
                //在这使用点击 alert(that.year + '-' + that.month + '-' + that.date);
                //that.removeDate();
            }
        }
    }
